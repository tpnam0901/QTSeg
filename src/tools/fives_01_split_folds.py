...
def main(args):
    ...
    target_paths = glob.glob(
        os.path.join(args.data_root, "train/Ground truth", "*.png")
    )
    target_paths.sort()
    sample_paths = [path.replace("Ground truth", "Original") for path in target_paths]
    ...
    os.makedirs(os.path.join(fold_root, "test", "inputs"), exist_ok=True)
    os.makedirs(os.path.join(fold_root, "test", "targets"), exist_ok=True)

    test_target_paths = glob.glob(
        os.path.join(args.data_root, "test/Ground truth", "*.png")
    )
    test_sample_paths = [
        path.replace("Ground truth", "Original") for path in target_paths
    ]
    for i in range(len(sample_paths)):
        assert os.path.exists(sample_paths[i]), "Sample path does not exist"
    for input_path, target_path in zip(test_sample_paths, test_target_paths):
        os.symlink(
            os.path.abspath(input_path),
            os.path.join(fold_root, "test", "inputs", os.path.basename(input_path)),
        )
        os.symlink(
            os.path.abspath(target_path),
            os.path.join(fold_root, "test", "targets", os.path.basename(target_path)),
        )
