...
def main(args):
    ...
    target_paths = glob.glob(os.path.join(args.data_root, "benign", "*_mask.png"))
    target_paths.sort()
    sample_paths = [path.replace("_mask", "") for path in target_paths]
    ...
    target_paths = glob.glob(os.path.join(args.data_root, "malignant", "*_mask.png"))
    target_paths.sort()
    sample_paths = [path.replace("_mask", "") for path in target_paths]
    for i in range(len(sample_paths)):
        assert os.path.exists(sample_paths[i]), "Sample path does not exist"
