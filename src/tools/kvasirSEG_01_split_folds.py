...
def main(args):
    ...
    target_paths = glob.glob(os.path.join(args.data_root, "masks", "*.jpg"))
    target_paths.sort()
    sample_paths = [path.replace("masks", "images") for path in target_paths]
...
def arg_parser():
    ...
    parser.add_argument(
        ...
        default="../working/dataset/Kvasir-SEG/",
    )
    parser.add_argument(
        "--num_folds",
        type=int,
